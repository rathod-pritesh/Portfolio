...
// parse a hex string into an ObjectID, returning a 400 error on failure.
...
func AdminAddProject(c *gin.Context) {
	...
	c.JSON(http.StatusCreated, gin.H{
		...
		"insertedId": newID.Hex(),
	})
}

// PUT
func AdminUpdateProject(c *gin.Context) {
	oid, ok := parseProjectID(c)
	if !ok {
		return
	}
...
func AdminReorderProject(c *gin.Context) {
	...
	for _, item := range items {
		oid, errr := primitive.ObjectIDFromHex(item.ID)
		if errr != nil {
			continue // skip invalid IDs
		}
		col.UpdateOne(ctx, bson.M{"_id": oid}, bson.M{"$set": bson.M{"order": item.Order}})
	}
